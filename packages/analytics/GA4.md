...
- `updateAccumulator` discards a delta of 30 minutes or more outright rather than clamping it. GA4
  has no such rule because its timer only runs while engaged. A page with no mouse, keyboard or
  scroll activity — a video playing — can lose the whole watch time.
- `startTime` is the last accumulation anchor, not when the session began, and is rewritten on
  every tick. The name predates the split.
