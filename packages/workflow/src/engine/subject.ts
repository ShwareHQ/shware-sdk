...
/**
 * Subject personalization: a subject is a string template whose `{prop}`
 * placeholders name user properties (compile-checked by `emailSubject` on the
 * DSL side). The engine fills them from the profile at send time — subjects
 * are data end to end, never closures.
 */
